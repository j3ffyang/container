...
Download airgap binary and place it into ```/var/lib/rancher/k3s/agent/images``` if the device doesn't have access to ```gcr.io``` and/ or ```googleapis.com```
...
2. If ```journalctl -u k3s-agent.service``` complains ```key not match to server...```. You can find the correct key on master
