...
- Gen
```sh
openssl req -x509 -new -nodes -key ca.key -subj "/CN=cptheat" -days 3650 -reqexts v3_req -extensions v3_ca -out ca.crt
...
```sh
openssl req -x509 -new -nodes -key ca.key -subj "/CN=cptheat" -days 3650 -out ca.crt
```
...
```sh
ubuntu@smoke01:~/cert-mgr$ cat issuer.yaml
...
- Create certs resource

```sh
ubuntu@smoke01:~/cert-mgr$ cat certs.yaml
apiVersion: cert-manager.io/v1alpha2
kind: Certificate
metadata:
  name: example-com
...
ubuntu@smoke01:~/cert-mgr$ k apply -f certs.yaml
```

- Create `ingress`

```sh
ubuntu@smoke01:~/ingress$ cat prom.yaml
